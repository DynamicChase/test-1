  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== 'seller' && user.role !== 'admin') {
        setError('This account is not registered as a seller. Please use the buyer login.');
        authApi.logout();
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold">Offer Services</CardTitle>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex flex-col gap-4">
            ...
            <div className="text-sm text-center text-muted-foreground">
              ...
              <Link to="/login/buyer" className="text-primary hover:underline font-medium">
                Login as buyer
              </Link>
            </div>
          </CardFooter>
      ...
      <div className="fixed bottom-4 right-4 p-4 bg-muted rounded-lg max-w-xs text-sm">
        <p className="font-semibold mb-2">Demo Seller Account:</p>
        <p className="text-xs text-muted-foreground">Email: john@example.com</p>
        <p className="text-xs text-muted-foreground">Password: seller123</p>
      </div>
    </div>
  );
}
